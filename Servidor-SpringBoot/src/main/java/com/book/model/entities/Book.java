...
@NamedQuery(name="Book.countBooks", query="SELECT count(title) FROM Book b where user_id = ?1 and state > 0 and (reserved = null or reserved > 0)")
@NamedQuery(name="Book.countBuyReservedBooks", query="SELECT count(title) FROM Book b where reserved = 1 and buyer_id != null and user_id = ?1")
